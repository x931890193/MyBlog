...
APlayer.prototype.init = function () {
    ...
    if (this.element = this.option.element, this.music = this.option.music, this.option.showlrc) {
        ...
        ////   /\[(\d{2}):(\d{2})\.([0-9]*)]/
        for (var l = this.element.getElementsByClassName("aplayer-lrc-content")[0].innerHTML, s = l.split(/\n/), n = /\[.*]/, r = /](.*)$/, o = 0; o < s.length; o++) {

            var d = n.exec(s[o]), c = r.exec(s[o]);
            // console.log(d,  'this is d');
            // console.log(c, 'this is c');
            // console.log(d[1], d[2], d[3]);
            // console.log(s[o], '数组 ', o);
            // console.log(r.exec(s[o]), '正则', o);
            // console.log(l,2222);
            // console.log(s[o],3333);
            // console.log(this.l/rcLine, o);
            // console.log(n);
            // console.log(d, c)
            //console.log(this.lrcTime, 'this.lrcTime')
            if (c, d) {

                var new_d = [];
                var new_str = d.toString().replace(/\d+/g, function () {

                //调用方法时内部会产生 this 和 arguments
                // console.log(arguments[0]);
                new_d.push(arguments[0]);

    });

                console.log(new_d[0],':', new_d[1], ':', new_d[2]);
                console.log(c[1]);
                new_d && c && (this.lrcTime.push(60 * parseInt(new_d[0]) + parseInt(new_d[1]) + parseInt(new_d[2]) / 100), this.lrcLine.push(c[1]))
            }
        }
    }
    ...
    this.audio.addEventListener("durationchange", function () {
        ...
        m.element.getElementsByClassName("aplayer-author")[0].innerHTML = " - " + m.music.author, m.loadedTime = setInterval(function () {
            if (!m.audio.buffered.length || !isFinite(m.audio.duration) || m.audio.duration <= 0) return;
